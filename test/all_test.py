...
if __name__ == "__main__":
    ...
    for file in modules:
        if (
            ...
            and os.path.join(cur_path, file) != os.path.abspath(__file__)
        ):
            name = file[:-3]
            if (
                onlys
                and name not in onlys
                and (name.endswith("_test") and name[:-5] not in onlys)
            ):
                print(f"Skipping {name}")
                continue
            if ignores and (
                name in ignores or (name.endswith("_test") and name[:-5] in ignores)
            ):
                print(f"Skipping {name}")
                continue
