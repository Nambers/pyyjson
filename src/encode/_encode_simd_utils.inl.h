...
force_inline void BACK_WRITE_SIMD256_WITH_TAIL_LEN(_TARGET_TYPE *dst, SIMD_256 y, Py_ssize_t len, void *head) {
...
#        define ELEVATOR PYYJSON_CONCAT5(elevate, COMPILE_READ_UCS_LEVEL, COMPILE_WRITE_UCS_LEVEL, to, SIMD_BIT_SIZE)
    ...
    SIMD_128 x1, x2;
    split_tail_len_two_parts(len, READ_BATCH_COUNT, &part1, &part2);
    ...
    if (COMPILE_WRITE_UCS_LEVEL == 4 || part1) {
        assert(COMPILE_WRITE_UCS_LEVEL == 4 || (uintptr_t)dst >= (uintptr_t)head);
...
#    if COMPILE_READ_UCS_LEVEL == 1 && COMPILE_WRITE_UCS_LEVEL == 4
    ...
    usize parts = (usize)size_align_up(len, READ_BATCH_COUNT / 4) / (READ_BATCH_COUNT / 4);
    SIMD_128 x;
    SIMD_512 to_write;
    switch (parts) {
        case 4: {
            ...
            write_512((void *)(dst + (READ_BATCH_COUNT / 4) * 3), to_write);
        }
        case 3: {
            x = SIMD_EXTRACT_QUARTER(z, 2);
            to_write = elevate_1_4_to_512(x);
            write_512((void *)(dst + (READ_BATCH_COUNT / 4) * 2), to_write);
        }
        case 2: {
            x = SIMD_EXTRACT_QUARTER(z, 1);
            to_write = elevate_1_4_to_512(x);
            write_512((void *)(dst + (READ_BATCH_COUNT / 4) * 1), to_write);
        }
        case 1: {
...
#        endif
    assert(len < READ_BATCH_COUNT && len > 0);
    usize parts = (usize)size_align_up(len, READ_BATCH_COUNT / 2) / (READ_BATCH_COUNT / 2);
    SIMD_256 y;
    SIMD_512 to_write;
    switch (parts) {
        case 2: {
            ...
            to_write = ELEVATOR(y);
            write_512((void *)(dst + (READ_BATCH_COUNT / 2) * 1), to_write);
        }
        case 1: {
